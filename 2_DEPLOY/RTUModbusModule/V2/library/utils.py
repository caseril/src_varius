...
def get_measurement_list_from_dict(machine_type, dictionary, parameters_keys=None, logger=None):
    ...
    prefix_classes_dict['FUNCTION_CHECK_']          =   library.measurement.FunctionCheckModbus
...
def get_all_measurement_list(machine_type, dictionary, parameters_keys=None):
    ...
    function_check_modbus_list = get_function_check_modbus_list(machine_type, dictionary, parameters_keys=parameters_keys)
    # adam_measurement_list = get_adam_measurement_list(machine_type, dictionary, parameters_keys=parameters_keys)
    return measurement_list + modbus_measurement_list + function_measurement_list + function_check_modbus_list # + adam_measurement_list
...
def get_function_check_modbus_list(machine_type, dictionary, prefix='FUNCTION_CHECK_', parameters_keys=None):
    import library.measurement
    measurement_list = []
    for key in sorted(dictionary): # sorted(dictionary) returns a list of keys sorted alphabetically
        if key.startswith(prefix):
            try:
                ...
                measurement_dict = json.loads(dictionary[key])
                sensor_type = list(measurement_dict.keys())[0]
                measurement = library.measurement.FunctionCheckModbus(machine_type, sensor_type, measurement_dict[sensor_type], dictionary, parameters_keys=parameters_keys)
                measurement_list.append(measurement)
            except Exception as e:
                logging.critical(F'EXCLUDING MEASUREMENT: {dictionary[key]}')
                logging.getLogger().debug(e, exc_info=True) 

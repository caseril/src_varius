...
class BaseModbus(ABC):
    ...
    async def execute_command_read(self, command):
        self.logger.info(f'execute_command_read::start::{command.register}::{command.index}')
        self.logger.debug(f'{json.dumps(command)}')
        if command.access == ModbusAccess.READ or command.access == ModbusAccess.READ_WRITE:
            ...
            value = await self.read_value(command.register, command.modbus_type.name, index=command.index, register_type=register_type, count=command.count)
    ...
    async def read_value(self, register_num, type_str, register_type=None, index = 0, count=None, array_count=1):
        ...
        if not self.is_simulator():
            if type_str in ModbusTypes.__members__:
                ...
                if value_type == ModbusTypes.COIL:
                    ...
                    values = []
                    for _ in range(array_count if index==0 else int((index)/2+1)):
                        if type_str.startswith('BIT'):
                            bits_high = decoder.decode_bits()
                            bits_low  = decoder.decode_bits()
                            bit_num   = int(type_str.replace('BIT', '').replace('HIGH', '').replace('LOW', ''))
                            
                            self.logger.info(f'read_value::bits_high::{bits_high}::bits_low::{bits_low}::bit_num::{bit_num}::index::{index}')
                    ...
                    return (values[0] if index is None else values[int(index/2)]) if array_count == 1 else values
            else:

...
class BaseModbus(ABC):
    ...
    async def execute_command_write(self, command, value):
        if command.access == ModbusAccess.WRITE or command.access == ModbusAccess.READ_WRITE:
            return await self.write_value(command.register, command.modbus_type.name, self._write_encode_(command, value))
    ...
    async def connect_to_modbus_server(self):
        if not self.is_simulator():
            logging.getLogger().debug('Connecting to modbus server')  # at {}.'.format(self.modbus_port))

            if (await utils.run_and_try_if_true_async(self.connect, 10, 0.1)):
                logging.getLogger().info('Connected to modbus ')  # {}.'.format(self.modbus_port))
            else:
                logging.getLogger().critical('Impossible to connect to modbus server')  # at {}.'.format(self.modbus_port))
        else:
            print('self.is_simulator(): ', self.is_simulator())
            logging.getLogger().info('SIMULATOR: skipping connect_to_modbus_server')

    async def close_modbus_client(self):
        if not self.is_simulator():
            await utils.run_and_try_async(self.client.close, 3, 0.1)
            logging.getLogger().debug('Connection to modbus client closed.')
        else:
            logging.getLogger().info('SIMULATOR: skipping close_modbus_client')
    ...
    def read_holding_registers(self, register, count):
        if not self.is_simulator():
            ...
            logging.getLogger().debug("READ HOLDING: number: {}, Count: {}, values: {}".format(register, count, registers))
            return registers
        else:
            logging.getLogger().info('SIMULATOR: skipping read_holding_registers')
    ...
    def read_input_registers(self, register, count):
        if not self.is_simulator():
            ...
            logging.getLogger().debug("READ INPUT: number: {}, Count: {}, values: {}".format(register, count, registers))
            return registers
        else:
            logging.getLogger().info('SIMULATOR: skipping read_input_registers')

    def read_coils(self, register, count):
        if not self.is_simulator():
            bits = self.client.read_coils(register, count=count, unit=self.modbus_id).bits
            logging.getLogger().info("READ COILS: number: {}, Count: {}, values: {}".format(register, count, bits))  # TODO
            return bits
        else:
            logging.getLogger().info('SIMULATOR: skipping read_holding_registers')
        ...
        if not self.is_simulator():
            if type_str in ModbusTypes.__members__:
                ...
                if value_type == ModbusTypes.COIL:
                    logging.getLogger().critical("WRITING values: {} to coil register number: {}".format(value, register_num))
                    ...
                    logging.getLogger().critical("WRITING values: {} to register number: {}".format(builder.to_registers(), register_num))
    ...
    async def read_value(self, register_num, type_str, register_type=None, count=None, array_count=1):
        if array_count is None:
            array_count = 1
        if not utils.parse_bool(os.getenv("SIMULATOR", default=False), default=False):
            if type_str in ModbusTypes.__members__:
                value_type = ModbusTypes[type_str]
                ...
                if value_type == ModbusTypes.COIL:
                    ...
                    logging.getLogger().info('COIL REGISTER_NUM = {}, COUNT: {}, bits: {}'.format(register_num, count, bits))
                    values = bits[:count]
                    ...
                    logging.info('REGISTER_NUM = {}, COUNT: {}, REGISTERS: {}'.format(register_num, count, registers))
                    ...
                    for _ in range(array_count):
                        if type_str.startswith('BIT'):
                            ...
                            logging.getLogger().critical(f"bits_high: {bits_high}. bits_low: {bits_low}. bit_num: {bit_num}")
                ...
                logging.getLogger().error('{} is NOT a valid ModbusTypes value'.format(type_str))
                return None
        else:
            logging.info('SIMULATOR: skipping read_value')
            return None

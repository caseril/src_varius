...
class InfluxDBModuleClass:
    ...
    def __init__(self, logger, config_dict=None, database=None, host=None, port=None, username=None, password=None, ssl:bool=None, verify_ssl:bool=None):
        self.logger = logger
        
        database = utils.get(config_dict, 'DATABASE')                               if config_dict is not None else database
        host = utils.get(config_dict, 'HOST')                                       if config_dict is not None else host
        port = utils.get(config_dict, 'PORT')                                       if config_dict is not None else port
        username = utils.get(config_dict, 'USERNAME')                               if config_dict is not None else username
        password = utils.get(config_dict, 'PASSWORD')                               if config_dict is not None else password
        ssl = str(utils.get(config_dict, 'SSL')).lower() == 'true'                  if config_dict is not None else ssl
        verify_ssl = str(utils.get(config_dict, 'VERIFY_SSL')).lower() == 'true'    if config_dict is not None else verify_ssl

        if username is None:
            self.client=DataFrameClient(host=host, port=port)
        else:
            self.client = DataFrameClient(host=host, port=port, username=username, password=password, ssl=ssl, verify_ssl=verify_ssl)
